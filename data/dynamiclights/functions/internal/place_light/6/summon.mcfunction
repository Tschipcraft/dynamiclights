...
execute align xyz if block ~ ~ ~ light as @e[type=minecraft:marker,tag=ts.dl.light,distance=..0.1,limit=1,sort=nearest] at @s run function dynamiclights:internal/place_light/6/prev_it/check

execute if score #light_exists ts.dl.settings matches 0 run function dynamiclights:internal/place_light/6/summon_new
